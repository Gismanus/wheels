<html dir="ltr" lang="ru"><!--<![endif]-->

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Магазин инвалидных колясок в Москве | Специализированные реабилитационные товары для инвалидов</title>


    <link href="styles/icons.css" type="text/css" rel="stylesheet">
    <link href="styles/bootstrap.css" type="text/css" rel="stylesheet">
    <link rel="shortcut icon" href="/images/favicon.svg">
    <link href="styles/main.css" type="text/css" rel="stylesheet">
    <link href="styles/slick.css" type="text/css" rel="stylesheet">
    <link href="styles/base.css" type="text/css" rel="stylesheet">
    <link href="styles/buttons.css" type="text/css" rel="stylesheet">

    <link href="styles/menu.css" type="text/css" rel="stylesheet">
    <link href="styles/header.css" type="text/css" rel="stylesheet">
    <link href="styles/breadcrumb.css" type="text/css" rel="stylesheet">
    <link href="styles/responsive.css" type="text/css" rel="stylesheet">
    <link href="styles/footer.css" type="text/css" rel="stylesheet">
    <link href="styles/popup.css" type="text/css" rel="stylesheet">
    <link href="styles/form_new.css" type="text/css" rel="stylesheet">
    <link href="styles/basket.css" type="text/css" rel="stylesheet">
    <link href="styles/form.css" type="text/css" rel="stylesheet">
    <link href="styles/form-success.css" type="text/css" rel="stylesheet">
    <link href="styles/catalog.css" type="text/css" rel="stylesheet">
    <link href="styles/cat-tabs.css" type="text/css" rel="stylesheet">
    <link href="styles/good.css" type="text/css" rel="stylesheet">
    <!-- <link href="styles/jq.css" type="text/css" rel="stylesheet"> -->

    <base href="#">

    <!-- <script src="/js/header.min.js"></script> -->
    <script src="/js/jQuery.js"></script>
    <style>
        .new-class {
            background: #000;
        }
    </style>

    <!-- <script src="buyoneclick.js" type="text/javascript"></script> -->

    <!-- <link rel="stylesheet" href="styles/boc.css" type="text/css"> -->
</head>

<body>

    <div></div>

    <!--HEADER-->

    <header id="header">
        <div class="header-box">
            <div class="header-top">
                <div class="logo">
                    <a href="/">
                        <img src="images/logo.svg" alt="">
                    </a>
                </div>
                <ul class="header-data">
                    <li>
                        <div class="header-location">Ваш город: <a href="/" data-id-popup="p-city">Москва</a></div>
                        <!-- <div class="open-location"><i class="icon-location"></i></div> -->
                    </li>
                    <li>
                        <a href="/basket.html" class="header-basket">
                            <i class="icon-basket"></i>
                            <span class="d-none d-lg-inline d-sm-none">Корзина:&nbsp;</span>
                            <div class="d-inline js-header-basket-inner">
                                <span class="price d-none d-lg-inline">0 руб.</span>
                                <span class="number">10</span>
                            </div>
                        </a>
                    </li>
                    <li class="visable-responsive">
                        <div class="open-search">
                            <i class="icon-search"></i>
                        </div>
                    </li>
                    <li class="visable-responsive">
                        <div class="open-menu"></div>
                    </li>
                </ul>
                <div class="header-search">
                    <form action="/search/" class="js-search">
                        <div id="search" class="form-header-search">
                            <input type="hidden" name="description" value="true">
                            <input type="text" name="search" value=""
                                placeholder="Салют, друг!"
                                class="form-control input-lg js-search-text">
                            <button type="button"><i class="icon-search"></i></button>
                        </div>
                        <div class="ajax-search-container"></div>
                    </form>
                </div>
            </div>
            <div class="header-content">
                <div class="header-content-overlow"></div>
                <span class="close-menu"></span>
                <div class="header-content-box">
                    <nav>
                        <ul class="menu">

                            <li>
                                <a href="/catalog.html" class="HasDrop">Каталог</a>
                                <div class="menu-drop">
                                    <div class="back-catalog close-catalog-first">
                                        <i class="icon-arrow"></i>
                                        назад
                                    </div>
                                    <div class="menu-drop-box">
                                        <div class="menu-drop-left">
                                            <ul class="menu-catalog">
                                                
                                                <li><a href="/" class="active">Технические средства реабилитации для взрослых</a></li>
                                                <li><a href="/">Подъёмники для инвалидов</a>
                                                </li>
                                                <li>
                                                    <a href="/">Респираторное оборудование</a>
                                                </li>
                                            </ul>
                                        </div>

                                        <div class="menu-drop-right">
                                            <div class="menu-catalog-tab">
                                                <div class="back-catalog close-catalog-second">
                                                    <i class="icon-arrow"></i>
                                                    назад
                                                </div>

                                                <div class="menu-catalog-tab-box active">
                                                    <div class="row">
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Автомобильные
                                                                        кресла</a>
                                                                </li>
                                                                <li><a href="/">Коляски
                                                                        для детей активного типа</a>
                                                                </li>
                                                                <li><a href="/">Коляски
                                                                        для детей с ДЦП</a>
                                                                </li>
                                                                <li><a href="/">Детские
                                                                        коляски с электроприводом</a>
                                                                </li>
                                                                <li><a href="/">Детские
                                                                        вертикализаторы</a>
                                                                </li>
                                                                <li><a href="/">Функциональные
                                                                        опоры</a>
                                                                </li>
                                                                <li><a href="/">Ходунки</a>
                                                                </li>
                                                                <li><a href="/">Велосипеды</a>
                                                                </li>
                                                                <li><a href="/">Для
                                                                        ванны</a>
                                                                </li>
                                                                <li><a href="/">Товары
                                                                        для позиционирования</a>
                                                                </li>
                                                                <li><a href="/">Реабилитационные
                                                                        костюмы</a>
                                                                </li>
                                                                <li><a href="/">Иппотренажёры</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Аппараты и
                                                                        тутора</a>
                                                                </li>
                                                                <li><a href="/">Тренажёры</a>

                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="menu-catalog-tab-box">
                                                    <div class="row">
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Активные</a>
                                                                </li>
                                                                <li><a href="/">Кресла-коляски
                                                                        с ручным приводом</a>
                                                                </li>
                                                                <li><a href="/">Кресло-коляски
                                                                        с электроприводом</a>
                                                                </li>
                                                                <li><a href="/">Скутеры</a>
                                                                </li>
                                                                <li><a href="/">Приставки
                                                                        для кресел-колясок</a>
                                                                </li>
                                                                <li><a href="/">Для
                                                                        пляжа</a>
                                                                </li>
                                                                <li><a href="/">Кресла-каталки</a>
                                                                </li>
                                                                <li><a href="/">Вертикализаторы</a>
                                                                </li>
                                                                <li><a href="/">Кровати
                                                                        для дома</a>
                                                                </li>
                                                                <li><a href="/">Оборудование
                                                                        с туалетом</a>
                                                                </li>
                                                                <li><a href="/">Противопролежневые
                                                                        подушки</a>
                                                                </li>
                                                                <li><a href="/">Ванна
                                                                        для инвалидов</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Весы
                                                                        для инвалидов</a>
                                                                </li>
                                                                <li><a href="/">Тренажёры</a>
                                                                </li>
                                                                <li><a href="/">Иппотерапия</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="menu-catalog-tab-box">
                                                    <div class="row">
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Вертикальные</a>
                                                                </li>
                                                                <li><a href="/">Наклонные</a>
                                                                </li>
                                                                <li><a href="/">Гусеничные</a>
                                                                </li>
                                                                <li><a href="/">Комнатные</a>
                                                                </li>
                                                                <li><a href="/">Для бассейна </a>
                                                                </li>
                                                                <li><a href="/">Для
                                                                        ванны</a>
                                                                </li>
                                                                <li><a href="/">Электрические</a>
                                                                </li>
                                                                <li><a href="/">Для
                                                                        дома</a>
                                                                </li>
                                                                <li><a href="/">Лестничные</a>
                                                                </li>
                                                                <li><a href="/">Мобильные</a>
                                                                </li>
                                                                <li><a href="/">Уличные</a>
                                                                </li>
                                                                <li><a href="/">Трансформеры</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">Для автомобилей</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="menu-catalog-tab-box">
                                                    <div class="row">
                                                        <div class="col-lg-6">
                                                            <ul class="menu-catalog-sub">
                                                                <li><a href="/">CPAP
                                                                        | BPAP аппараты</a>
                                                                </li>
                                                                <li><a href="/">Аппараты
                                                                        для вентиляции лёгких</a>
                                                                </li>
                                                                <li><a href="/">Кислородное
                                                                        оборудование</a>
                                                                </li>
                                                                <li><a href="/">Откашливатели</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                            </li>
                            <li><a href="/">Производители</a></li>
                            <li><a href="/good-item">Образец товара</a></li>
                            <li><a href="/">Контакты</a></li>
                            <li><a href="/basket.html">Корзина</a></li>
                        </ul>
                    </nav>
                    <div class="header-contact">
                        <div class="header-phone"><a href="/">8 (999) 999-99-99</a></div>
                        <div class="header-phone"><a href="/">8 (999) 999-99-99</a></div>
                        <div class="header-time">
                            <i class="icon-time"></i>
                            Только в понедельник
                        </div>
                        <div class="header-contact-button">
                            <button data-id-popup="p-callback" class="btn btn-small">Не получать консультацию</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </header>
    <!--END HEADER-->

    <div id="main">
        <main class="content" id="common-home">
            <section class="section">

            </section>
        </main>
        <!--FOOTER-->
        <footer id="footer" class="footer">
            <div class="footer-box">
                <div class="footer-top">
                    <div class="row">
                        <div class="col-xl-3 col-lg-3 col-sm-5 col-xs-12 order-xs-1">
                            <div class="mb-4">
                                <a href="/">

                                    <img src="images/logo.svg" class="footer-logo" alt="" data-original-src="">
                                </a>
                            </div>
                            <div class="mb-4">
                                <div class="footer-phone"><a href="#">8 (999) 999-99-99</a></div>
                                <div class="footer-phone"><a href="#">8 (999) 999-99-99</a></div>
                            </div>
                            <div class="footer-contact">
                                <address class="footer-address">
                                    999999, г. Москва,<br> какой-то адрес
                                </address>
                                <div class="mb-4">
                                    e-mail: <a href="#">Какая-то почта</a>
                                </div>
                            </div>
                            <div class="mb-4"><button data-id-popup="p-callback" class="btn btn-small">Обратный
                                    звонок</button></div>
                            <ul>
                                <li><a href="/" class="footer-title">О компании</a></li>
                                <li><a href="/" class="footer-title">Акции</a></li>
                                <li><a href="/" class="footer-title">Производители</a></li>
                                <li><a href="/" class="footer-title">Доставка и оплата</a></li>
                            </ul>

                        </div>
                        <div class="col-xl-3 col-lg-3 col-sm-5 col-xs-12 order-xs-3">
                            <div class="mb-2"><a href="/" class="footer-title">ТСР
                                    для взрослых</a></div>
                            <ul class="mb-4">
                                <li><a href="/">Активные</a>
                                </li>

                            </ul>
                        </div>
                        <div class="col-xl-3 col-lg-3 col-sm-5 col-xs-12 d-flex flex-column order-xs-4">
                            <div class="mb-2"><a href="/" class="footer-title">Респираторное оборудование</a></div>
                            <ul class="mb-4">
                                <li><a href="/">CPAP |
                                        BPAP аппараты</a>
                                </li>
                                <li><a href="/">Аппараты
                                        для вентиляции лёгких</a>
                                </li>
                                <li><a href="/">Кислородное
                                        оборудование</a>
                                </li>
                                <li><a href="/">Откашливатели</a>
                                </li>
                            </ul>
                            <div class="mb-2"><a href="/" class="footer-title">Подъёмники для
                                    инвалидов</a></div>
                            <ul class="mb-4">
                                </li>
                                <li><a href="/">Мобильные</a>
                                </li>
                                <li><a href="/">Уличные</a>
                                </li>
                                <li><a href="/">Трансформеры</a>
                                </li>
                                <li><a href="/">Для автомобилей</a>
                                </li>
                            </ul>
                            <div class="mt-auto mb-2 d-flex footer-social">
                                <div class="mr-2">Для<br>взрослых </div>
                                <a href="/" target="_blank"><i class="icon-tg"></i></a>

                            </div>
                        </div>
                        <div class="col-xl-3 col-lg-3 col-sm-5 col-xs-12 d-flex flex-column order-xs-2">
                            <div class="mb-2"><a href="/" class="footer-title">ТСР для детей</a></div>
                            <ul class="mb-4">
                                <li><a href="/">Ходунки</a>
                                </li>
                                <li><a href="/">Велосипеды</a>
                                </li>
                                <li><a href="/">Для ванны</a>
                                </li>
                                <li><a href="/">Товары для
                                        позиционирования</a>
                                </li>
                                <li><a href="/">Реабилитационные костюмы</a>
                                </li>
                                <li><a href="/">Иппотренажёры</a>
                                </li>
                                <li><a href="/">Аппараты и тутора</a>
                                </li>
                                <li><a href="/">Тренажёры</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="footer-bottom pb-5">

                </div>
            </div>
        </footer>
        <!--END FOOTER-->
    </div>
    <!-- modals -->
    <div class="popup" id="p-tocart">
        <div class="popup-box">
            <a href="/" class="close-popup"><i class="icon-close"></i></a>
            <div class="popup-thx"></div>
            <div class="text-center">
                <button class="btn btn-small link-close-popup mb-2">Продолжить покупки</button>
                <a href="/" class="btn btn-small mb-2">Оформить заказ</a>
            </div>
        </div>
    </div>
    <div class="popup" id="p-callback">
        <div class="popup-box">
            <a href="/" class="close-popup"><i class="icon-close"></i></a>
            <div class="popup-title">Обратный звонок</div>
            <form action="/" method="post" class="ajaxform" data-ya="обратный звонок"
                data-error="Ошибка отправки! Попробуйте ещё раз!" data-ok="Спасибо, ваше сообщение отправлено!"
                data-invalid="Неверно заполнены данные!">
                <div class="alert alert-danger none"></div>
                <div class="alert alert-success none"></div>
                <input type="hidden" name="formid" value="1">
                <input type="hidden" name="utm" value="">
                <input type="hidden" name="work" value="">
                <div class="form-callback">
                    <div class="title text-center">Оставьте телефон и мы вам перезвоним</div>
                    <div class="form-callback-box row">
                        <div class="col-sm-3">
                            <label>ФИО:</label>
                        </div>
                        <div class="col-sm-9">
                            <div class="form-element">
                                <input type="text" name="fio" class="tx" placeholder="Иван Иванов">
                                <span class="text-error">не корректно введен</span>
                            </div>
                        </div>
                    </div>
                    <div class="form-callback-box row">
                        <div class="col-sm-3">
                            <label>Телефон:</label>
                        </div>
                        <div class="col-sm-9">
                            <div class="form-element">
                                <input type="text" name="phone" class="tx tx-phone" placeholder="+7 (999) 999-99-99"
                                    required="">
                                <span class="text-error">не корректно введен</span>
                            </div>
                        </div>
                    </div>
                    <div class="form-button">
                        <button class="btn btn-small">отправить</button>
                    </div>
                </div>
            </form>
        </div>
    </div>
    <div class="popup" id="p-city">
        <div class="popup-box">
            <a href="#" class="close-popup"><i class="icon-close"></i></a>
            <div class="popup-title">Ваш город:</div>
            <div class="form-header-search mb-3">
                <input id="autocomplete-cities" type="text" placeholder="Введите название города" autocomplete="off">
                <button><i class="icon-search"></i></button>
            </div>
            <div class="row">
                <div class="col-4"><a href="/">Москва</a></div>
                <div class="col-4"><a href="/">Санкт-Петербург</a></div>
                <div class="col-4"><a href="/">Казань</a></div>
                <div class="col-4"><a href="/">Новосибирск</a></div>
                <div class="col-4"><a href="/">Краснодар</a></div>
                <div class="col-4"><a href="/">Екатеринбург</a></div>
                <div class="col-4"><a href="/">Хабаровск</a></div>
                <div class="col-4"><a href="/">Челябинск</a></div>
                <div class="col-4"><a href="/">Ростов на Дону</a></div>
                <div class="col-4"><a href="/">Самара</a></div>
                <div class="col-6"><a href="/">Нижний Новгород</a></div>
                <div class="col-6"><a href="/">Крым (Симферополь)</a></div>
                <div class="col-12"><a href="/">Ставрапольский край (Пятигорск)</a></div>
                <div class="col-12"><a href="/">Алматы</a></div>
                <div class="col-12"><a href="/">Минск</a></div>
            </div>
        </div>
    </div>
    <script src="js/initMobile.js" type="text/javascript" async=""></script>
    <script src="js/ajaxHandler.js" type="text/javascript" async=""></script>
</body>

</html>